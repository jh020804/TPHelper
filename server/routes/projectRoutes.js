...
router.get('/:projectId', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const [teamMembers] = await connection.execute('SELECT u.id, u.name, u.email FROM project_members pm JOIN users u ON pm.user_id = u.id WHERE pm.project_id = ?', [projectId]);
...
router.post('/:projectId/invite', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (existing.length > 0) return res.status(409).json({ message: '이미 멤버임' });

        await connection.execute('INSERT INTO project_members (project_id, user_id, role, status) VALUES (?, ?, ?, ?)', [projectId, userId, 'member', 'active']);
...
router.post('/:projectId/chat', authMiddleware, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ id: result.insertId, project_id: projectId, user_id: userId, user_name: senderName, content, timestamp: new Date() });
    } catch (error) {
